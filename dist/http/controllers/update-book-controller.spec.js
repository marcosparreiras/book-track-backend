...
// src/http/controllers/update-book-controller.spec.ts
...
describe("PUT /book/:bookId", () => {
  ...
  it("Should be able to update a book data with admin credentials", () => __async(exports, null, function* () {
    const email = "johndoe@example.com";
    const password = "123456";
    const {
      body: { token }
    } = yield (0, import_supertest.default)(app).post("/session").send({ email, password });
    const response = yield (0, import_supertest.default)(app).put(`/book/${bookId}`).set({ Authorization: `Bearer ${token}` }).send({
  ...
  it("Should not be able to update a book with normal user credentials", () => __async(exports, null, function* () {
