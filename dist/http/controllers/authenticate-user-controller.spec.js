...
// src/http/controllers/authenticate-user-controller.spec.ts
...
describe("POST /session", () => {
  ...
  it("Should be able to create a session with user credentials", () => __async(exports, null, function* () {
    const name = "John Doe";
    const email = "johndoe@example.com";
    const password = "123456";
    yield (0, import_supertest.default)(app).post("/users").send({ name, email, password });
    const response = yield (0, import_supertest.default)(app).post("/session").send({ email, password });
    expect(response.status).toEqual(201);
    expect(response.body.token).toEqual(expect.any(String));
  }));
  it("Should not be able to create a session with invalid credentials", () => __async(exports, null, function* () {
    const requestBody = {
      email: "johndoe@example.com",
      password: "123456"
    };
    const response = yield (0, import_supertest.default)(app).post("/session").send(requestBody);
