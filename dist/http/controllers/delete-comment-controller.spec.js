...
// src/http/controllers/delete-comment-controller.spec.ts
...
describe("DELETE /book/:bookId/comment", () => {
  ...
  it("Should be able to delete a comment", () => __async(exports, null, function* () {
    ...
    const createCommentResponse = yield (0, import_supertest.default)(app).post(`/book/${bookId}/comment`).set({ Authorization: `Bearer ${token}` }).send({
      content: "Some fake comment",
      rate: 4
    });
    const response = yield (0, import_supertest.default)(app).delete(`/comment/${createCommentResponse.body.commentId}`).set({ authorization: `Bearer ${token}` });
