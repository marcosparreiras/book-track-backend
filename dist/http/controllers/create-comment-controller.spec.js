...
// src/http/controllers/create-comment-controller.spec.ts
...
describe("POST /book/:bookId/comment", () => {
  ...
  it("Should be able to create an comment on a book", () => __async(exports, null, function* () {
    const email = "johndoe@example.com";
    const password = "123456";
    const {
      body: { token }
    } = yield (0, import_supertest.default)(app).post("/session").send({ email, password });
    const response = yield (0, import_supertest.default)(app).post(`/book/${bookId}/comment`).set({ Authorization: `Bearer ${token}` }).send({
