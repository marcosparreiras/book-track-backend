...
// src/http/controllers/update-user-avatar-controller.spec.ts
...
describe("PATCH /me/avatar", () => {
  ...
  it("Should be able to update an avatar of an authenticated user", () => __async(exports, null, function* () {
    const name = "John Doe";
    const email = "johndoe@example.com";
    const password = "123456";
    yield (0, import_supertest.default)(app).post("/users").send({ name, email, password });
    const loginResponse = yield (0, import_supertest.default)(app).post("/session").send({ email, password });
    const token = loginResponse.body.token;
    const upldateAvatarResponse = yield (0, import_supertest.default)(app).patch("/me/avatar").set({ authorization: `Bearer ${token}` }).attach("avatar", "assets/test-avatar.png");
    expect(upldateAvatarResponse.status).toEqual(204);
    const getProfileResponse = yield (0, import_supertest.default)(app).get("/me").set({ authorization: `Bearer ${token}` });
